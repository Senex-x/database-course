...
FROM flights
         INNER JOIN (
    SELECT MAX(duration) AS max_duration_column
    FROM flights
) AS max_duration_table ON duration = max_duration_table.max_duration_column;

-- Task 4

-- Queries are split into subexpressions for easier readability step-by-step
-- Get all flights for each airport
SELECT airports.id AS airport_id, flights.id AS flight_id, flights.date AS flight_date
FROM airports
         JOIN flights
...
SELECT passenger_id, sum(duration) AS total_duration
FROM (
         SELECT passenger_id, flights.duration
         FROM (
                  SELECT passenger_id, flight_id
...
FROM passengers
    ...
    FROM total_durations
             JOIN (
        SELECT MAX(total_duration) AS max_duration, MIN(total_duration) AS min_duration
        FROM total_durations
    ) AS max_duration_table
                  ON total_duration = max_duration
                      OR total_duration = min_duration
) AS extremum_total_durations ON id = extremum_total_durations.passenger_id
...
-- Для каждого пилота вывести цепочку городов, по которым он летал. Пример:
